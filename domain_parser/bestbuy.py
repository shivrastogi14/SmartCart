...
filename = os.path.join(credentials, 'parameters.ini')
...
def login_check(driver, pay: bool):
    if pay is False:
        try:
...
def dismiss_sign_up(driver):
    dismiss: bool = driver.find_element(By.CSS_SELECTOR, "input[type='submit']") \
                    and item.find_element(By.CSS_SELECTOR, "title[type='Sign Up']")
    if dismiss:
        item.find_element(By.CLASS_NAME, 'c-close-icon.c-modal-close-icon').click()
...
def login_and_pay(driver):
    try:
        ...
        login_check(driver, True)
        login_and_pay(driver)
...
#             WebDriverWait(driver, 4).until(
#                 expect.presence_of_element_located((By.CLASS_NAME, 'v-fw-medium.prefilled-label')))
#             correct_email = driver.find_element(By.CLASS_NAME, 'prefilled-value').text
...
def payment_information(driver):
    try:
        ...
        street = driver.find_element(By.CSS_SELECTOR, 'input[id = "city"]')
    ...
    try:
        ...
        for itm in items:
            ...
            if itm.get_attribute("auto-test-sku") != '6424521':
                ...
                quantity_button = itm.find_element_by_css_selector('select[class = "tb-select"')
